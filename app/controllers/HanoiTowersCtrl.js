;(function(){
	...
	HanoiCtrl.$inject=['$scope','$location','ConfigSvc'];
	
	
	function HanoiCtrl($scope,$location,ConfigSvc){
		console.log(ConfigSvc.Hanoi.Config);
		$scope.title="Towers of Hanoi";
		var myLays = $location.search().layers;
		console.log(myLays);
		if (myLays){
			$scope.Layers=myLays;
		}else{$scope.Layers=ConfigSvc.Hanoi.Config.Layers;}
